...
PRETRAINED_PATH = FLAGS.pretrained_path if FLAGS.pretrained_path is not None \
    else DEFAULT_CHECKPOINT_PATH
...
def load_backbone(model, weights):
    ...
    filtered_weights = {
        k: v for k, v in weights.items() if k.startswith('backbone_net.') and v.size() == model_state[k].size()
    }
    finetune_unique_weights = {
        k: v for k, v in model_state.items() if not k.startswith('backbone_net.') or v.size() != weights[k].size() 
    }
...
if PRETRAINED_PATH is not None and os.path.isfile(PRETRAINED_PATH):
    ...
    state = torch.load(PRETRAINED_PATH, map_location=lambda s, l: default_restore_location(s, 'cpu'))['model_state_dict']
