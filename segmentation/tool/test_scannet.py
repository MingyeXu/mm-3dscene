...
def data_prepare():
    ...
    if args.data_name == 's3dis':
        ...
        # get data_name
        # print('0000000')
        data_path = os.path.join(args.data_root, 'scannet_train_detection_data')
        all_scan_names = list(set([os.path.basename(x)[0:12] \
            for x in os.listdir(data_path) if x.startswith('scene')]))
        
        if args.test_split in ['val', 'test']:
            split_filenames = os.path.join(args.data_root, 'meta_data',
                'scannetv2_{}.txt'.format(args.test_split))
            with open(split_filenames, 'r') as f:
                scan_names = f.read().splitlines()   
            # remove unavailiable scans
            num_scans = len(scan_names)
            data_list = [sname for sname in scan_names \
...
def data_load(data_name):
    if args.data_name == 's3dis':
        ...
        data_path = os.path.join(args.data_root, 'scannet_train_detection_data')
        vert_path = os.path.join(data_path, data_name + '_vert.npy')
        sem_path = os.path.join(data_path, data_name + '_sem_label.npy')
        ins_path = os.path.join(data_path, data_name + '_ins_label.npy')
