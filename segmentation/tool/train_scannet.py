...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/scannet/scannet_pointtransformer_repro.yaml', help='config file')
    parser.add_argument('opts', help='see config/scannets/cannet_pointtransformer_repro.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, eta_min=0.0001, last_epoch=-1, verbose=False)
    ...
    train_data = ScanNet(split='train', data_root=args.data_root,  voxel_size=args.voxel_size, voxel_max=args.voxel_max, transform=train_transform, shuffle_index=True, loop=args.loop)
    if main_process():
    ...
    if args.evaluate:
        val_transform = None
        val_data = ScanNet(split='val', data_root=args.data_root, voxel_size=args.voxel_size, voxel_max=80000, transform=val_transform)
        if args.distributed:
